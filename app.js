...
function renderMore(){const items=[['calendar','▦','Calendario'],['targets','◎','Target'],['commissions','€','Commissioni'],['simulator','◇','Simulatore'],['transfer','⇅','Import / Export'],...(state.role==='admin'?[['settings','⚙','Impostazioni']]:[])];$('#main').innerHTML=head('Altro','Strumenti e configurazione')+`<div class="more-grid">${items.map(([id,ico,l])=>`<button class="more-item" data-sub="${id}"><span>${ico}</span><b>${l}</b></button>`).join('')}</div>`;$$('[data-sub]').forEach(b=>b.onclick=()=>renderSub(b.dataset.sub))}
